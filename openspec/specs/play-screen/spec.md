...
<!-- @trace
source: add-end-of-hand-view
updated: 2026-05-03
code:
  - internal/play/play.go
  - cmd/play.go
  - internal/play/endpanel.go
tests:
  - internal/play/play_test.go
-->
...
For pon, chi, minkan, and shouminkan, the seat-source marker SHALL be attached immediately to the LEFT of the called tile within the meld — the tile most recently grabbed for that meld — rather than as a meld-level prefix. The called tile is the tile at index 0 for pon and minkan (the engine places the discard first), at the last index for chi (the engine appends the discard after the chi pair) and for shouminkan (the engine appends the upgrade tile after the original pon trio). This places the marker as a pointer at the physical tile that came from another seat, so the player can see at a glance which tile in the meld is the called one.

For ankan there is no called tile (all four come from the concealed hand). The `[A]` marker SHALL render as a meld-level prefix immediately before the first tile.
...
#### Scenario: Pon meld marker points at called tile from East

- **GIVEN** the human's open-melds list contains a single `MeldPon{Tiles: [5p, 5p, 5p], From: SeatEast}` (the engine constructs pon tiles as `[discard, discard, discard]`, so the called tile is at index 0)
- **WHEN** the play screen renders
- **THEN** the rendered output contains the substring `[E]` immediately preceding the first `5p` glyph (the called tile), positioned to the right of the concealed-hand row

#### Scenario: Chi meld marker points at the appended discard tile

- **GIVEN** the human's open-melds list contains a single `MeldChi{Tiles: [2m, 3m, 4m], From: SeatWest}` (chi tiles are `[c1, c2, discard]`, so the called tile is the last)
- **WHEN** the play screen renders
- **THEN** the rendered output contains `2m`, then `3m`, then `[W]`, then `4m` in that left-to-right order — the marker pointing at the called 4m, NOT preceding the entire meld

#### Scenario: Ankan meld renders with [A] as a meld-level prefix

- **GIVEN** the human's open-melds list contains a single `MeldKan{KanKind: KanAnkan, Tiles: [1m, 1m, 1m, 1m]}`
- **WHEN** the play screen renders
- **THEN** the rendered output contains `[A]` immediately preceding the first `1m` glyph (no per-tile pointer because ankan has no called tile)
...
- **WHEN** the play screen renders
- **THEN** the rendered output contains all three meld blocks separated by single-tile-width gaps, in order: `[E]5p 5p 5p`, `2m 3m [W]4m`, `[A] 1z 1z 1z 1z` — pon and chi markers point at the called tile, ankan marker is a meld-level prefix
